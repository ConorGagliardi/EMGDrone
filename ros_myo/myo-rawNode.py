...
#obtained from https://github.com/uts-magic-lab/ros_myo
#this code itself is based on another myo utility called 'pyomyo'
#modified by John Pesarchick 5/6/2023 to collect raw, unmodified EMG signals and perform classification. 
...
class MyoRaw(object):
    ...
    def connect(self):
        ...
        if self.old:
            ...
            # enable EMG data
            self.write_attr(0x28, b'\x01\x00') #Default: x01 for the higher byte
            # enable IMU data
...
#NEW BLOCK---------------------------------------------------------------------            
                ...
                emg = struct.unpack('<8b', pay[8:]) #Taken from pyomyo: see https://github.com/PerlinWarp/pyomyo/blob/main/src/pyomyo/pyomyo.py
                #When using the raw data samples, it seems two samples are sent at once. 
                moving = 0 #from observation, this value always seemed to be 0 for the rectified data, so it's just hardcoded here. 
                # not entirely sure what the last byte is,
                # but it's a bitmask that seems to indicate which
                # sensors think they're being moved around or something
                #emg = vals[:8]
...
#NEW BLOCK---------------------------------------------------------------------
    ...
    def start_raw(self):
        ...
        #These four are all new...
        self.write_attr(0x2c, b'\x01\x00')
        self.write_attr(0x2f, b'\x01\x00')
        self.write_attr(0x32, b'\x01\x00')
        self.write_attr(0x35, b'\x01\x00')
        #self.write_attr(0x19, b'\x01\x03\x01\x01\x01') Old value- this enables a stream of data that is both filtered and rectified. Useless for our purposes...
        self.write_attr(0x19, b'\x01\x03\x03\x01\x01') #This one enables raw data, which is what we need. 

    def mc_start_collection(self):
        '''Myo Connect sends this sequence (or a reordering) when starting data
        collection for v1.0 firmware; this enables raw data but disables arm
        and pose notifications.'''
        #This is the same as the pyomyo function
        self.write_attr(0x28, b'\x01\x00')
        self.write_attr(0x1d, b'\x01\x00')
        self.write_attr(0x24, b'\x02\x00')
...
if __name__ == '__main__':
    ...
    def proc_imu(quat1, acc, gyro):
        ...
        roll, pitch, yaw = euler_from_quaternion([normQuat.x,
                                                  ...
                                                  normQuat.z,
                                                  normQuat.w])
        normGyro = Vector3(roll, pitch, yaw)
        #imu = Imu(h, normQuat, cov, normGyro, cov, normAcc, cov)
        #imuPub.publish(imu)
